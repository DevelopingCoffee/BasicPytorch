...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        # input_size: the number of features for each time stamp
        # for batch_first, the shape must be: [batch, time_seq, features]
        ...
        # using just last hidden-state, as it contains all infos necessary:
    ...
    def forward(self, x):
        ...
        # LSTM:
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)  # cell state for LSTM only
...
# model = RNN(input_size, hidden_size, num_layers, num_classes)  # Input will be 28x28=784, output-dim = 10
